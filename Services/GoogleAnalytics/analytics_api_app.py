...
#  return None
  ...
  for property in properties.get('items', []):
     firstAccountId = property.get('accountId')
     firstWebpropertyId = property.get('id')
     profiles = service.management().profiles().list(
         accountId=firstAccountId,
         webPropertyId=firstWebpropertyId).execute()
     ids[property.get('name')] = profiles.get('items')[0].get('id')
  return ids
...
def get_top_keywords(service, profile_id):
  ...
  res = {}
  for k,v in profile_id.iteritems():
      res[k] = service.data().realtime().get(
                ids='ga:' + v,
